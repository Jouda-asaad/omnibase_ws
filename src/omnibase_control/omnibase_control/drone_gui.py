...
class DroneControlGUI(Node):
    def __init__(self):
        ...
        self.is_landed = False  # Track if drone has touched down
        
        # Publishers
        self.cmd_vel_pub = self.create_publisher(Twist, '/uav/cmd_vel', 10)
        self.landing_pub = self.create_publisher(Bool, '/landing_status', 10)
        
        # Timer to continuously publish landing status
        self.status_timer = self.create_timer(0.1, self.publish_landing_status)
    ...
    def publish_landing_status(self):
        """Publish landing status for base_tracker"""
        msg = Bool()
        msg.data = self.is_landing
    ...
    def start_landing(self):
        ...
        if self.is_aligned:
            ...
            self.get_logger().info('Landing initiated! Base will now stop.')
        else:
            self.get_logger().warn('Cannot land - not aligned!')
    
    def reset_landing(self):
        """Reset landing state to allow taking off again"""
        self.is_landing = False
        self.is_landed = False
        # Lift drone up first before tracking resumes
        self.get_logger().info('Taking off! Lifting drone...')
        for _ in range(10):  # Send multiple lift commands
            self.send_velocity(z=0.3)
    ...
    def create_gui(self):
        ...
        # Take Off Button (appears after landing)
        self.takeoff_button = tk.Button(self.root, text="🚀 TAKE OFF", 
                                        font=('Arial', 18, 'bold'),
                                        bg='#666666', fg='white',
                                        width=15, height=2,
                                        state='disabled',
                                        command=self.reset_landing)
    ...
    def update_gui(self):
        ...
        if self.is_landing:
            ...
            # TAKE OFF is always available during landing to abort/reset
            self.takeoff_button.config(state='normal', bg='#0066aa')
            ...
            self.takeoff_button.config(state='disabled', bg='#666666')
        else:
